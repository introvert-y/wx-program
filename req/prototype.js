...
function requestP(options = {}) {
  ...
  // 统一注入约定的header
  const header = { ...{ sessionId }, ...options.header };
    ...
    wx.login({
      success(r1) {
        if (r1.code) {
          // 获取sessionId
          requestP({
            url: `${config.apiUrl}/business-api/api/sys/login`,
            method: 'POST',
            data: {
              // jsCode: r1.code,
              jsCode: '011bNXFC0ODH4l2bewIC0GtSFC0bNXFM',
            },
          })
            .then((r2) => {
              if (r2.code === 0) {
...
function uploadFile(data) {
  const url = `${config.apiUrl}/business-api/api/file/upload`;
  return new Promise((res, rej) => {
    getSessionId()
      .then(() => {
        wx.uploadFile({
          ...
          formData: {
            type: data.type, // 1考勤 2绑定银行卡 3实名认证 4公司logo
          },
          name: 'file',
          header: {
            sessionId,
